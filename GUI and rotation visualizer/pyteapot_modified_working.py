...
def read_data():
    ...
    if(useQuat):
        w = float(line.split('c')[1])
        nx = float(line.split('w')[1])
        ny = float(line.split('a')[1])
        nz = float(line.split('b')[1])
        return [w, nx, ny, nz]
    else:
        yaw = float(line.split('y')[1])
        pitch = float(line.split('p')[1])
...
def draw(w, nx, ny, nz):
    ...
    if(useQuat):
        ...
        glRotatef(2 * math.acos(w) * 180.00/math.pi, -1 * nx, nz, ny)        #modified
    else:
        yaw = nx
        pitch = ny
        roll = nz
...
def quat_to_ypr(q):
    ...
    yaw = math.asin(2*(q0*q2-q3*q1))
    roll  = math.atan2(2*(q0*q3+q1*q2),1-2*(q2*q2+q3*q3))
    pitch *= 180.0 / math.pi
    yaw   *= 180.0 / math.pi
    yaw   -= 3  # Declination at Chandrapur, Maharashtra is - 0 degress 13 min
    roll  *= 180.0 / math.pi
    return [yaw, pitch, roll]
